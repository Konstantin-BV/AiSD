...
struct Node
{
    ...
    int value;
    string key;
    Node(Node* l, Node* r, Node* p, string k, int v) : left(l), right(r), parent(p), color('r'), key(k), value(v){}
...
public:
    ...
    void Insert(Node* PARENT, Node* ROOT, string key,int value)
    {
        if (!root)
        {
            root = new Node(NULL, NULL, PARENT, key, value);
            Repainting(root);
        }
        else
        {
            ...
            if (key > PARENT->key)
            {
                if (!ROOT->right)
                {
                    ...
                    ROOT->right = new Node(NULL, NULL, ROOT,key, value);
                    insert_case1(ROOT->right);
                    // fixViolation(root, ROOT->right);
                }
                else
                {
                    Insert(ROOT, ROOT->right, key,value);
            ...
            else
            {
                if (!ROOT->left)
                {
                    ROOT->left = new Node(NULL, NULL, ROOT, key, value);
